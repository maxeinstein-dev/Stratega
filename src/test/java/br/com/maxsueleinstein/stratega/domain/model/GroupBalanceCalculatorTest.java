...
class GroupBalanceCalculatorTest {
    ...
    void shouldCalculateSimpleBalances() {
        ...
        GroupExpense expense = new GroupExpense(UUID.randomUUID(), "Almoço", new BigDecimal("100.00"), alice, java.time.LocalDateTime.now(), splits, new UniformSplitStrategy(), "EXPENSE");
    ...
    void shouldCalculateComplexBalances() {
        ...
        expenses.add(new GroupExpense(UUID.randomUUID(), "E1", new BigDecimal("90.00"), alice, java.time.LocalDateTime.now(), splits1, new UniformSplitStrategy(), "EXPENSE"));
        
        // 2. Bob pagou 60, dividido por Alice, Bob (30 cada)
        List<Split> splits2 = List.of(new UniformSplit(alice), new UniformSplit(bob));
        expenses.add(new GroupExpense(UUID.randomUUID(), "E2", new BigDecimal("60.00"), bob, java.time.LocalDateTime.now(), splits2, new UniformSplitStrategy(), "EXPENSE"));
